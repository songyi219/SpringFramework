...
if(list == null || list.size() == 0){
	...
	for(int i = 0;i < list.size(); i++)
	{
		...
		%>
		<tr>
			<th><%=i + 1 + (pageNumber * 10) %></th>	<!-- 글이 10개씩 보이게 -->
			
			<%
			if(dto.getDel() == 0){		// 삭제되지 않은 글
				%>			
				<td>
				...
				<%
			}else if(dto.getDel() == 1){		// del값을 1으로 바꿔줘서 삭제된 글
				%>
				<td>
					<%=Utility.arrow(dto.getDepth()) %>
...
$('#pagination').twbsPagination({
    ...
    onPageClick: function (event, page) {
    	...
    	let choice = document.getElementById('choice').value;
		let search = document.getElementById('search').value;
    	location.href = "bbslist.do?choice=" + choice + "&search=" + search + "&pageNumber=" + (page-1); // 0부터 돌아가기때문	
